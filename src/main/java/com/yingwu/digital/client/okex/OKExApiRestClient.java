...
import com.yingwu.digital.util.MD5Util;
...
public class OKExApiRestClient {
    ...
    public OKExApiRestClient(String url_prex,String api_key,String secret_key){
        this.api_key = api_key;
        this.secret_key = secret_key;
        this.url_prex = url_prex;
    }

    public OKExApiRestClient(String url_prex){
        this.url_prex = url_prex;
    }
    ...
    public String ticker(String symbol) throws HttpException, IOException {
        ...
        String param = "";
        if(!DigitalStringUtils.isEmpty(symbol )) {
            if (!param.equals("")) {
                param += "&";
            }
            param += "symbol=" + symbol;
    ...
    public String trades(String symbol, String since) throws HttpException, IOException {
        ...
        if(!DigitalStringUtils.isEmpty(since )) {
            if (!param.equals("")) {
                param += "&";
            }
            param += "since=" + since;
    ...
    public String trade(String symbol, String type,
        ...
        if(!DigitalStringUtils.isEmpty(type)){
            params.put("type", type);
        }
        if(!DigitalStringUtils.isEmpty(price)){
            params.put("price", price);
        }
        if(!DigitalStringUtils.isEmpty(amount)){
    ...
    public String batch_trade( String symbol, String type,
        ...
        if(!DigitalStringUtils.isEmpty(orders_data)){
    ...
    public String cancel_order(String symbol, String order_id) throws HttpException, IOException {
        ...
        if(!DigitalStringUtils.isEmpty(order_id)){
    ...
    public String order_history(String symbol, String status,
        ...
        if(!DigitalStringUtils.isEmpty(status)){
            params.put("status", status);
        }
        if(!DigitalStringUtils.isEmpty(current_page)){
            params.put("current_page", current_page);
        }
        if(!DigitalStringUtils.isEmpty(page_length)){
