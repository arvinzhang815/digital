...
public class IdUtil {
    ...
    private static final Map<Integer, IdUtil> instanceMap = new HashMap<Integer, IdUtil>();

    // 必须提供正确的参数,以保证 suffix 在集群环境的唯一性
    private IdUtil(final int suffix) {
        IdUtil.suffix = suffix;
    }
     ...
     * @return 主键生成器实例
     */
    public synchronized static IdUtil getInstance() {
     ...
     * @return 主键生成器实例
     */
    public synchronized static IdUtil getInstance(int suffix) {
        IdUtil pkgen;
        if (!instanceMap.containsKey(suffix)) {
            pkgen = new IdUtil(suffix);
